...
export async function assessSeverity(disruption) {
  ...
  if (isClaudeAvailable()) {
    const systemPrompt = `You are a supply chain risk analyst specialising in Indian logistics corridors. Assess the severity of disruptions on Indian freight routes. Respond ONLY with a JSON object (no markdown, no explanation) with fields: severity (low/medium/high/critical), reasoning (2-3 sentences, vary your phrasing), riskScore (1-10), recommendation (1 sentence).`;
...
- Category: ${disruption.category}`;
    ...
    if (parsed && parsed.severity && parsed.reasoning) {
